...
class lm:
    ...
    def fit(self, lss):
        ...
        self.p_values = [2 * (1 - stats.t.cdf(np.abs(i), (len(self.x) - 1))) for i in self.ts_b]
        #
...
pvals_ = res.pvalues
print('pvalues')
print('{A:15.8f} {B:15.8f} -- {S}'.format(A=pvals[0], B=pvals_[0], S=pvals[0] == pvals_[0]))
print('{A:15.8f} {B:15.8f} -- {S}'.format(A=pvals[1], B=pvals_[1], S=pvals[1] == pvals_[1]))
print('{A:15.8f} {B:15.8f} -- {S}'.format(A=pvals[2], B=pvals_[2], S=pvals[2] == pvals_[2]))
print('Coeffcients:')
